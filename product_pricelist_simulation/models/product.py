    ...
    def _compute_price(self):
        for simulation in self:
            simulation.price = simulation.product_tmpl_id.with_context(
                pricelist=simulation.pricelist_id.id
            ).price

    product_tmpl_id = fields.Many2one(
        comodel_name="product.template", string="Product", index=True,
    )
    pricelist_id = fields.Many2one(
        comodel_name="product.pricelist", string="Pricelist", index=True,
    )
            ...
            for pricelist in pricelists:
                ...
                if not simulation:
                    res += simulation.create(
                        {"product_tmpl_id": template.id, "pricelist_id": pricelist.id,}
                    )
                else:
                    res += simulation
