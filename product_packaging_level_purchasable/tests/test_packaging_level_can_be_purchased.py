...
class TestPackagingLevelCanBePurchased(Common):
    ...
    def test_product_packaging_can_be_purchased(self):
        """Check that a product.packaging can be independently set as can be
        purchased."""
        exception_msg = (
            f"Packaging Test packaging cannot be purchased on product "
            f"{self.product.name} must be set "
        ...
        # Packaging can be purchased even if the packaging level does not allow it
        ...
        # Changing the can_be_purchased on the packaging_level does not update the
        # packaging
