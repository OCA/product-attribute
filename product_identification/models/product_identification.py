...
class ProductIdentification(models.Model):
    _name = "product.identification"
    _description = "Product Identification"

    name = fields.Char(string="ID Number", required=True)
    category_id = fields.Many2one(
        "product.identification.category", string="Category", required=True
    )
