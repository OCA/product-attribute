...
class product_index(osv.osv):
    def _current_rate(self, cr, uid, ids, name, arg, context=None):
        ...
        for id in ids:
            cr.execute("""
SELECT index_id, rate
FROM product_index_rate
WHERE index_id = %s AND name <= '%s'
ORDER BY name desc LIMIT 1""" % (id, date))
    ...
    def round(self, cr, uid, index, amount, context=None):
...
select rate
from product_index_rate
where name<=%s and index_id=%s
order by name desc limit 1''', (date_from, index.id))
        ifrom = cr.rowcount and cr.fetchone()[0] or 1.0
        cr.execute('''
select rate
from product_index_rate
where name<=%s and index_id=%s
order by name desc limit 1''', (date_to, index.id))
...
class product_index(osv.osv):
    ...
    def _current_rate(self, cr, uid, ids, name, arg, context=None):
        ...
        for product in self.browse(cr, uid, ids, context):
            ...
            for i in ifields:
                val = self.pool.get('product.index').compute(
                    cr, uid, i, val, product.index_date
                )
            res[product.id] = val
        return res
    ...
    _columns = {
        ...
        'index_sale': fields.many2many(
            'product.index',
            'product_index_sale_rel',
            'product_id',
            'index_id',
            'Sales indexes'
        ),
        'index_purchase': fields.many2many(
            'product.index',
            'product_index_purchase_rel',
            'product_id',
            'index_id',
            'Purchase indexes'
        ),
        'index_date': fields.date('Index price date', required=True),
        'list_price_index': fields.function(
            _current_rate,
            method=True,
            string='Indexed list price',
            digits=(12, 6)
        ),
        'standard_price_index': fields.function(
            _current_rate,
            method=True,
            string='Indexed standard price',
            digits=(12, 6)
        ),
        'buyer_price_index': fields.function(
            _current_rate,
            method=True,
            string='Indexed buyer price',
            digits=(12, 6)
        ),
    }
    _defaults = {
