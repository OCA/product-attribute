...
class product_index(osv.osv):
    ...
    def _current_rate(self, cr, uid, ids, name, arg, context={}):
        ...
        for id in ids:
            cr.execute(
                "SELECT index_id, rate FROM product_index_rate WHERE index_id = %s AND name <= '%s' ORDER BY name desc LIMIT 1" % (id, date))
            if cr.rowcount:
    ...
    def compute(self, cr, uid, index, amount, date_from, date_to=None, round=True, context={}):
        ...
        cr.execute(
            'select rate from product_index_rate where name<=%s and index_id=%s order by name desc limit 1', (date_from, index.id))
        ifrom = cr.rowcount and cr.fetchone()[0] or 1.0
        cr.execute(
            'select rate from product_index_rate where name<=%s and index_id=%s order by name desc limit 1', (date_to, index.id))
