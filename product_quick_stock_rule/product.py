...
class product_product(Model):
    ...
    def _set_min_stock(self, cr, uid, product_id, name, value, arg,
        ...
        if op_ids:
            orderpoint_obj.write(
                cr, uid, op_ids[0], {'product_min_qty': value}, context=context
            )
        elif value:
            vals = self._prepare_orderpoint_minrule(
                cr, uid, product_id, value, context=context
            )
            vals['active'] = False
    ...
    def _prepare_orderpoint_minrule(self, cr, uid, product_id, min_qty=0,
                                    context=None):
        orderpoint_obj = self.pool.get('stock.warehouse.orderpoint')
        context['default_product_id'] = product_id
        res = orderpoint_obj.default_get(
            cr, uid, orderpoint_obj._columns.keys(), context=context
        )
        res['product_min_qty'] = min_qty
        return res

    def _set_rule_status(self, cr, uid, product_id, name, value, arg,
                         context=None):
        orderpoint_obj = self.pool.get('stock.warehouse.orderpoint')
        op_ids = orderpoint_obj.search(
            cr, uid, [
        ...
        if op_ids:
            orderpoint_obj.write(
                cr, uid, op_ids[0], {'active': value}, context=context
            )
        elif value:
            vals = self._prepare_orderpoint_minrule(
    ...
    _columns = {
        'active_rule': fields.function(
            _get_rule_status,
            fnct_inv=_set_rule_status,
            type='boolean',
            string='Active Rule',
        ),
        'qty_min': fields.function(
            _get_min_stock,
            fnct_inv=_set_min_stock,
            type='float',
            string='Minimal Stock'
        ),
    }
...
class stock_warehouse_orderpoint(Model):
    ...
    def default_get(self, cr, uid, fields, context=None):
        defaults = super(stock_warehouse_orderpoint, self).default_get(
            cr, uid, fields, context=context
        )
        if ('product_uom' in fields
                and not defaults.get('product_uom')
                and defaults.get('product_id')):
            prod = self.pool.get('product.product').browse(
                cr, uid, defaults['product_id'], context=context
            )
            defaults['product_uom'] = prod.uom_id.id
        if ('warehouse_id' in fields
                and not defaults.get('location_id')
                and defaults.get('warehouse_id')):
            w = self.pool.get('stock.warehouse').browse(
                cr, uid, defaults['warehouse_id'], context=context
            )
