...
class ResProduct(models.Model):
    ...
    def browse(self, arg=None, prefetch=None):
        for tab in self._get_tabs():
            ...
            if fieldname not in self._fields:
                ...
                self.add_field(tab)
        return super(ResProduct, self).browse(arg=arg, prefetch=prefetch)
    ...
    def fields_view_get(
        ...
        result = super(ResProduct, self).fields_view_get(
    ...
    def _add_tab_pages(self, view):
        """Adds the relevant tabs to the product's formview."""
        # pylint: disable=no-member
    ...
    def _get_tabs(self):
        tab_model = self.env['product.tab']
