...
class ProductBattery(models.Model):
    ...
    lead_exceed = fields.Boolean(
        string="Exceeds Lead by 0.004%",
        help="Identifies if the battery exceeds strict heavy metal limits",
    )
    cadmium_exceed = fields.Boolean(
        string="Exceeds Cadmium by 0.002%",
        help="Identifies if the battery exceeds strict heavy metal limits",
    )
    mercury_exceed = fields.Boolean(
        string="Exceeds Mercury by 0.0005%",
        help="Identifies if the battery exceeds strict heavy metal limits",
    )
        ...
        for record in self:
            if record.battery_voltage != 0:
                record.battery_capacity = (
                    record.battery_charge_capacity / 1000
