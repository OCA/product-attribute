...
class product_icecat_mapline(osv.osv):
    ...
    _defaults = {
        'model_id': lambda self, cr, uid, c: self.pool.get('ir.model').search(
...
class product_icecat(osv.osv):
        ...
        for id in ids:
            ...
            try:
                ...
                raise osv.except_osv(
                    _('Error !'),
                    _("IP FTP connection was not successfully!")
                )
            ...
            raise osv.except_osv(
                _('Ok !'),
                _("FTP connection was successfully!")
            )
    ...
    _columns = {
        ...
        'mapline_ids': fields.one2many(
            'product.icecat.mapline',
            'icecat_id',
            'Mapline',
        ),
        'ftp': fields.boolean('Active'),
        'ftpip': fields.char('IP', size=256),
        'ftpdirectory': fields.char(
            'Directory',
            size=256,
            help='If not use directory, insert . (point). '
                 'If use directory, path FTP dir'
        ),
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if vals.get('active'):
            ...
            if len(actv_ids):
                raise osv.except_osv(
                    _('Error!'),
                    _('They are other icecat configuration with "Active" '
                      'field checked. Only one configuration is avaible for '
                      'active field.')
                )
