...
class product_icecat_wizard(osv.osv_memory):
    ...
    _columns = {
        ...
        'description_sale': fields.boolean('Description Sale'),
    ...
    def icecat2oerp(self, cr, uid, form, product, icecat, pathxml, language,
        ...
        for prod in doc.xpathEval('//ProductFeature'):
            prodFeatureId.append(
                prod.xpathEval('@CategoryFeatureGroup_ID')[0].content +
        ...
        for feature in prodFeatureId:
            if not prodFeatureName[i] == 'Source data-sheet':
                ...
                prodFeature[values[0]].append(
                    '<strong>' + prodFeatureName[i] + ':</strong>' + ' ' +
                    value
                )
            i += 1
        ...
        for mapline_id in mapline_ids:
            mapline = self.pool.get('product.icecat.mapline').browse(
                cr, uid, mapline_id
            )
            ...
            if catID in prodFeature and len(prodFeature[catID]):
                for mapline_field in mapline_fields:
                    if mapline_field['icecat'] == catID:
                        ...
                        mapline_values.append({
                            'field': mapline_field['oerp'],
                            'source': source,
                        })
                    # This is not hardcode. Short description is available in
                    # another fields, for example meta_description website
        ...
        if form.name:
            trans_name_id = self.pool.get('ir.translation').search(
                cr, uid, [
                    ('lang', '=', language),
        ...
        if form.description_sale:
            trans_descsale_id = self.pool.get('ir.translation').search(
                cr, uid, [
                    ('lang', '=', language),
        ...
        if form.description:
            ...
            trans_description_id = self.pool.get('ir.translation').search(
                cr, uid, [
                    ('lang', '=', language),
    ...
    def iceimg2oerpimg(self, cr, uid, form, product, icecat, pathxml, data,
        ...
        if image:
            ...
            return _("Not exist %s image") % pathxml

    # ==========================================
    # wizard
    # =========================================
    def import_xml(self, cr, uid, ids, data, context=None):
        if context is None:
            context = {}
        ...
        if not form.language_id:
            language = self.pool.get('res.users').browse(
                cr, uid, uid
            ).context_lang
        ...
        for prod in data['active_ids']:
            ...
            if ean:
                url = 'http://data.icecat.biz/xml_s3/xml_server3.cgi?' \
                      'ean_upc=%s;lang=%s;output=productxml' % (ean, lang)
                fileName = '%s.xml' % ean
