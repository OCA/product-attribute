    ...
    def icecat2oerp(self, cr, uid, form, product, icecat, pathxml, language, data, context):
                ...
                for mapline_field in mapline_fields:
                    if mapline_field['icecat'] == catID:
                        ...
                        mapline_values.append(
                            {'field': mapline_field['oerp'], 'source': source})
                    ...
                    if mapline_field['icecat'] == 'ShortSummaryDescription':
                        mapline_values.append(
                            {'field': mapline_field['oerp'], 'source': short_description})
        ...
        if form.name:
            trans_name_id = self.pool.get('ir.translation').search(cr, uid, [(
                'lang', '=', language), ('name', '=', 'product.template,name'), ('res_id', '=', product.id)])
            if trans_name_id:
        ...
        if form.description_sale:
            trans_descsale_id = self.pool.get('ir.translation').search(cr, uid, [(
                'lang', '=', language), ('name', '=', 'product.template,description_sale'), ('res_id', '=', product.id)])
        ...
        if form.description:
            ...
            trans_description_id = self.pool.get('ir.translation').search(cr, uid, [(
                'lang', '=', language), ('name', '=', 'product.template,description'), ('res_id', '=', product.id)])
