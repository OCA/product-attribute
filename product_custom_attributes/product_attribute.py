...
class product_attribute(Model):
    ...
    def create(self, cr, uid, vals, context=None):
        if vals.get('based_on') == 'product_template':
            vals['model_id'] = self.pool.get('ir.model').search(
                cr, uid, [('model', '=', 'product.template')], context=context
            )[0]
            serial_name = 'attribute_custom_tmpl'
        else:
            vals['model_id'] = self.pool.get('ir.model').search(
                cr, uid, [('model', '=', 'product.product')], context=context
            )[0]
            serial_name = 'attribute_custom_variant'
        if vals.get('serialized'):
            vals['serialization_field_id'] = (
                self.pool.get('ir.model.fields').search(
                    cr, uid, [('name', '=', serial_name)], context=context
                )[0]
            )
        if vals['attribute_type'] == 'select':
            ...
            if not vals.get('serialized'):
                raise except_osv(
                    _('Create Error'),
                    _("The field serialized should be ticked for a "
                      "multiselect field !")
                )
        else:
    ...
    def onchange_field_description(self, cr, uid, ids, field_description,
        ...
        if field_description:
            name = unidecode(
                'x_%s' % field_description.replace(' ', '_').lower()
            )
