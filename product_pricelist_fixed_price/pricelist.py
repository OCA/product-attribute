...
Please create or activate one."""
        ...
        for product_id, qty, partner in products_by_qty_by_partner:
            for pricelist_id in pricelist_version_ids:
                ...
                tmpl_id = (
                    products_dict[product_id].product_tmpl_id
                    and products_dict[product_id].product_tmpl_id.id
                    or False
                )

                categ_id = (
                    products_dict[product_id].categ_id
                    and products_dict[product_id].categ_id.id
                    or False
                )
...
ORDER BY sequence""", (tmpl_id, product_id, plversion_ids[0], qty))
                ...
                for res in res1:
                    if res:
                        if res['base'] == -1:
                            if not res['base_pricelist_id']:
                                ...
                                price = currency_obj.compute(
                                    cr, uid, ptype_src, res['currency_id'],
                            ...
                            if sinfo:
                                ...
                                product_default_uom = product_obj.read(
                                    cr, uid, [tmpl_id], ['uom_id']
                                )[0]['uom_id'][0]
                                seller_uom = supplierinfo_obj.read(
                                    cr, uid, sinfo, ['product_uom']
                                )[0]['product_uom'][0]
                                if (seller_uom and product_default_uom
                                        and product_default_uom != seller_uom):
                                    uom_price_already_computed = True
                                    qty_in_product_uom = \
                                        product_uom_obj._compute_qty(
                                            cr, uid, product_default_uom, qty,
                        ...
                        # False means no valid line found ! But we may not
                        # raise an exception here because it breaks the search
                        price = False
