...
class product_product(orm.Model):
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        if not default:
            default = {}
        original = self.read(
            cr, uid, id, fields=['default_code', 'image_ids'], context=context)
        default.update({
            'image_ids': False,
        })
        local_media_repository = self.pool.get(
            'res.company').get_local_media_repository(cr, uid, context=context)
        ...
        if vals.get('default_code') and ids:
            ...
            if local_media_repository:
                old_product = self.read(
                    cr, uid, ids[0], ['default_code', 'image_ids'], context=context)
                ...
                if old_product['image_ids']:
                    if old_product['default_code'] != vals['default_code']:
                        ...
                        if os.path.isdir(old_path):
                            os.rename(
                                old_path,  os.path.join(local_media_repository, vals['default_code']))
                return res
    ...
    def create_image_from_url(self, cr, uid, id, url, image_name=None, context=None):
        ...
        new_image_id = self.pool.get('product.images').create(
