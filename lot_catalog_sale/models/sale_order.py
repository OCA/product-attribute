...
class SaleOrder(models.Model):
    _inherit = ["sale.order", "lot.catalog.mixin"]
    _name = "sale.order"
    ...
    def _get_action_add_from_stock_catalog_extra_context(self):
        return {
            **super()._get_action_add_from_stock_catalog_extra_context(),
    ...
    def _get_lot_catalog_order_domain(self):
    ...
    def _get_lot_catalog_domain(self):
        extra_domain = self._get_lot_catalog_order_domain()
        return expression.AND([super()._get_lot_catalog_domain(), extra_domain])

    def _get_lot_catalog_record_lines(self, lot_ids, **kwargs):
    ...
    def _get_sol_from_lot_catalog(self, lot_id):
    ...
    def _prepare_line_vals_from_lot_catalog(self, lot, qty, **kwargs):
        return {
            "order_id": self.id,
    ...
    def _update_lot_order_line_info(self, lot_id, quantity, **kwargs):
        ...
        sol = self._get_sol_from_lot_catalog(lot_id)
        if sol:
            if quantity != 0:
            ...
            sol_vals = self._prepare_line_vals_from_lot_catalog(lot, quantity, **kwargs)
...
class SaleOrderLine(models.Model):
    ...
    def _get_lot_catalog_lines_data(self, **kwargs):
        product = self.product_id
        res = {
            "quantity": self.product_uom_qty,
            "price": self._get_discounted_price(),
            "readOnly": (
                self.order_id._lot_is_readonly() or product.sale_line_warn == "block"
            ),
        }
        if (
    ...
    def action_add_from_stock_catalog(self):
        ...
        return order.action_add_from_stock_catalog()
