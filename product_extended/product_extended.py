...
class product_supplierinfo(osv.osv):
    ...
    def _last_order(self, cr, uid, ids, name, arg, context=None):
        ...
        for supinfo in self.browse(cr, uid, ids, context=context):
            cr.execute("""\
select po.id, max(po.date_approve)
from purchase_order as po, purchase_order_line as line
where po.id=line.order_id
  and line.product_id=%s
  and po.partner_id=%s
  and po.state='approved'
group by po.id""", (supinfo.product_id.id, supinfo.name.id,))
            record = cr.fetchone()
            if record:
    ...
    def _last_order_date(self, cr, uid, ids, name, arg, context):
        ...
        dates = po.read(cr, uid, filter(None, last_orders.values()),
                        ['date_approve'])
        for suppinfo in ids:
            date_approve = [
                x['date_approve']
                for x in dates
                if x['id'] == last_orders[suppinfo]
            ]
            if date_approve:
    ...
    _columns = {
        ...
        'last_order_date': fields.function(
            _last_order_date,
            type='date',
            method=True,
...
WHERE product_id=%s''', (product_id, ))
    ...
    def _product_dispo(self, cr, uid, ids, name, arg, context=None):
        if context is None:
            context = {}
        res = {}
    ...
    _columns = {
        'calculate_price': fields.boolean(
            'Compute standard price',
            help="Check this box if the standard price must be computed from "
                 "the BoM."
        ),
    ...
    def _calc_price(self, cr, uid, bom):
        if not bom.product_id.calculate_price:
            ...
            if bom.bom_lines:
                ...
                if no_child_bom and bom.id not in no_child_bom:
                    ...
                    if not other_bom.product_id.calculate_price:
                        price += self._calc_price(cr, uid, other_bom) * \
                            other_bom.product_qty
                    else:
            ...
            if bom.bom_lines:
                self.write(
                    cr, uid, [bom.product_id.id],
                    {'standard_price': price / bom.product_qty}
                )
