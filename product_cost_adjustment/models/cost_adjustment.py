...
class CostAdjustment(models.Model):
    ...
    product_ids = fields.Many2many(
        ...
        "Only shows products that have the product category set to 'manual' valuation.",
    )
    ...
    def _remove_unchanged_lines(self):
        for line in self.line_ids:
            if line.product_cost == line.product_original_cost:
                line.unlink()
        return True
    ...
    def action_open_cost_adjustment_lines(self):
        ...
        action["view_id"] = self.env.ref(
            "product_cost_adjustment.cost_adjustment_line_view_tree"
        ).id
