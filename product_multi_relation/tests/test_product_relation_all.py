...
class TestProductRelation(TestProductRelationCommon):
    ...
    def setUp(self):
        ...
        category_nothing = self.category_model.create({
            'name': 'Nothing'})
        (self.type_nothing,
         self.selection_nothing,
         self.selection_nothing_inverse) = (
            self._create_relation_type_selection({
                'name': 'has relation with nothing',
                'name_inverse': 'nothing has relation with',
                'product_type_left': 'consu',
                'product_type_right': 'service',
                'product_category_left': category_nothing.id,
                'product_category_right': category_nothing.id}))
    ...
    def test_get_product_types(self):
        """Product types should contain at least 'consu' and 'service'."""
        product_types = self.selection_model.get_product_types()
        type_codes = [ptype[0] for ptype in product_types]
        self.assertTrue('consu' in type_codes)
        self.assertTrue('service' in type_codes)
    ...
    def test_on_change_type_selection(self):
        ...
        self.assertTrue(
            ('type', '=', 'service') in domain['other_product_id'])
        # 3. Test with relation needing categories,
        #    take active product from active_id:
        relation_consu_service = self.relation_all_model.with_context(
            active_id=self.product_03_cat_sd.id).create({
                'type_selection_id': self.selection_sd2ss.id,
                'other_product_id': self.product_04_cat_ss.id})
        domain = relation_consu_service.onchange_type_selection_id()['domain']
        self.assertTrue(
            ('categ_id', '=', self.category_01_sd.id) in
            domain['this_product_id'])
        self.assertTrue(
            ('categ_id', '=', self.category_02_ss.id) in
            domain['other_product_id'])
        # 4. Test with invalid or impossible combinations
        relation_nothing = self._get_empty_relation()
        with self.env.do_in_draft():
            relation_nothing.type_selection_id = self.selection_nothing
        warning = relation_nothing.onchange_type_selection_id()['warning']
        self.assertTrue('message' in warning)
        self.assertTrue('No this product available' in warning['message'])
        with self.env.do_in_draft():
            relation_nothing.this_product_id = self.product_02_consumable
        warning = relation_nothing.onchange_type_selection_id()['warning']
        self.assertTrue('message' in warning)
        self.assertTrue('incompatible' in warning['message'])
        # Allow left product and check message for other product:
        self.type_nothing.write({'product_category_left': False})
        self.selection_nothing.invalidate_cache(ids=self.selection_nothing.ids)
        warning = relation_nothing.onchange_type_selection_id()['warning']
        self.assertTrue('message' in warning)
    ...
    def test_on_change_product_id(self):
        ...
        self.assertTrue(
            ('product_type_this', '=', 'consu') in domain['type_selection_id'])
        # 3. Test with invalid or impossible combinations
        relation_nothing = self._get_empty_relation()
        with self.env.do_in_draft():
            relation_nothing.this_product_id = self.product_02_consumable
            relation_nothing.type_selection_id = self.selection_nothing
        warning = relation_nothing.onchange_product_id()['warning']
        self.assertTrue('message' in warning)
        self.assertTrue('incompatible' in warning['message'])
    ...
    def test_write(self):
        ...
        (type_nservice2consumable,
         selection_nservice2consumable,
         selection_consumable2nservice) = self._create_relation_type_selection({
             'name': 'is servicing',
             'name_inverse': 'is serviced by',
             'product_type_left': 'service',
             'product_type_right': 'consu'})
        relation_consumable2service.write({
            'this_product_id': product_extra_service.id,
            'type_selection_id': selection_nservice2consumable.id,
        ...
        self.assertEqual(
            ...
            selection_nservice2consumable.id)
        self.assertEqual(
