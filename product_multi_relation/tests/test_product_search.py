...
class TestProductSearch(TestProductRelationCommon):
    ...
    def test_search_product_category(self):
        ...
        relation_sd_ss = self.relation_all_model.create({
            'this_product_id': self.product_03_cat_sd.id,
            'type_selection_id': self.selection_sd2ss.id,
            'other_product_id': self.product_04_cat_ss.id,
        })
        self.assertTrue(relation_sd_ss)
        products = self.product_model.search([
            ('search_relation_product_categ_id', '=',
             self.category_02_ss.id)
        ])
        self.assertTrue(self.product_03_cat_sd in products)
