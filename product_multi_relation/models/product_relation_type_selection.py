...
product_relation_type where each type is included in the
...
class ProductRelationTypeSelection(models.Model):
    """Virtual relation types"""
    _name = 'product.relation.type.selection'
    ...
    product_type_this = fields.Selection(
        selection='get_product_types',
        string='Current record\'s product type',
    )
    is_inverse = fields.Boolean(
        string="Is reverse type?",
        help="Inverse relations are from right to left product.",
    )
    product_type_other = fields.Selection(
        selection='get_product_types',
        string='Other record\'s product type',
    )
    product_category_this = fields.Many2one(
        comodel_name='product.category',
        string='Current record\'s category',
    )
    product_category_other = fields.Many2one(
        comodel_name='product.category',
...
CREATE OR REPLACE VIEW %(table)s AS
 ...
 SELECT
     ...
     name AS name,
     False AS is_inverse,
     product_type_left AS product_type_this,
     product_type_right AS product_type_other,
     product_category_left AS product_category_this,
     product_category_right AS product_category_other
 ...
 JOIN product_relation_type typ ON (bas.type_id = typ.id)
     %(additional_tables)s
            ...
            {'table': AsIs(self._table),
             'underlying_table': AsIs('product_relation_type'),
             'additional_view_fields':
                AsIs(self._get_additional_view_fields()),
             'additional_tables':
                 AsIs(self._get_additional_tables())})
        return super(ProductRelationTypeSelection, self)._auto_init()
