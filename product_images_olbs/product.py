...
class product_product(Model):
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        if not default:
            default = {}
        original = self.read(cr, uid, id, fields=['default_code', 'image_ids'],
                             context=context)
        default.update({
            'images_ids': False,
        })
            ...
            if original['image_ids']:
                ...
                if os.path.isdir(old_path):
                    try:
                        ...
                        logger.exception(
                            'error while trying to copy images from %s to %s',
                            old_path, old_path + '.copy'
                        )
    ...
    def write(self, cr, uid, ids, vals, context=None):
        if isinstance(ids, (int, long)):
            ids = [ids]
        # here we expect that the write on default_code is always on 1 product
        # because there is an unique constraint on the default code
        if vals.get('default_code', False) and ids:
            local_media_repository = self.pool.get('res.company').\
                get_local_media_repository(cr, uid, context=context)
            if local_media_repository:
                old_product = self.read(
                    cr, uid, ids[0], ['default_code', 'image_ids'],
                    context=context
                )
                ...
                if old_product['image_ids']:
                    if old_product['default_code'] != vals['default_code']:
                        old_path = os.path.join(
                            local_media_repository, old_product['default_code']
                        )
                        if os.path.isdir(old_path):
                            os.rename(old_path, os.path.join(
                                local_media_repository, vals['default_code'])
                            )
                return res
        return super(product_product, self).write(cr, uid, ids, vals,
                                                  context=context)

    def create_image_from_url(self, cr, uid, id, url, image_name=None,
                              context=None):
        ...
        data = {
            'name': image_name or filename,
            'extention': extention,
            'file': img,
            'product_id': id,
        }
        self.pool.get('product.images').create(cr, uid, data, context=context)
        return True
