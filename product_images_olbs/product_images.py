...
class product_images(Model):
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        return super(product_images, self).create(
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if vals.get('name', False) or vals.get('extention', False):
            ...
            for image in images:
                old_full_path = self._image_path(cr, uid, image,
                                                 context=context)
                ...
                if vals.get('name', False) and (image.name != vals['name']) \
                        or vals.get('extention', False)\
                        and (image.extention != vals['extention']):
                    super(product_images, self).write(
                        cr, uid, image.id, vals, context=context)
                    upd_ids.remove(image.id)
                    if 'file' in vals:
                        # a new image have been loaded we should remove the
                        # old image
                        # TODO it's look like there is something wrong with
                        # function field in openerp indeed the preview is
                        # always added in the write :(
                        if os.path.isfile(old_full_path):
                            os.remove(old_full_path)
                    else:
                        new_image = self.browse(cr, uid, image.id,
                                                context=context)
                        new_full_path = self._image_path(cr, uid, new_image,
                                                         context=context)
                        # we have to rename the image on the file system
                        if os.path.isfile(old_full_path):
                            os.rename(old_full_path, new_full_path)
        return super(product_images, self).write(cr, uid, upd_ids, vals,
                                                 context=context)
        ...
        if image.link:
            ...
            try:
                if isinstance(image.product_id.default_code, bool):
                    _logger.debug('product not completely setup, no image '
                                  'available')
                    full_path = False
                else:
                    full_path = self._image_path(cr, uid, image,
                                                 context=context)
            except Exception as e:
                _logger.error("Can not find the path for image %s: %s", id, e,
                              exc_info=True)
                return False
            if full_path:
    ...
    def _set_image(self, cr, uid, id, name, value, arg, context=None):
        ...
        return self.write(
            cr, uid, id, {'file_db_store': value}, context=context
        )
    ...
    _columns = {
        ...
        'link': fields.boolean(
            'Link?',
            help="Images can be linked from files on your file system or "
                 "remote (Preferred)"
        ),
