    ...
    def _compute_available_product_lot_sequence_ids(self):
        for rec in self:
            if rec.product_id:
                ...
                rec.available_product_lot_sequence_ids = False
                rec.available_product_lot_sequence_count = 0

    @api.onchange("available_product_lot_sequence_ids")
    def onchange_available_product_lot_sequence_ids(self):
        self.ensure_one()
        if self.product_id:
            if len(self.available_product_lot_sequence_ids) == 1:
                self.product_lot_sequence_id = self.env["product.lot.sequence"].browse(
                    self.available_product_lot_sequence_ids.ids
                )
