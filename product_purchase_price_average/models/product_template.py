...
class ProductTemplate(models.Model):
    ...
    def update_average_purchase_price(self, templates):
        ...
        if is_multicompany_env:
            shared_tmpl = templates.filtered(lambda r: not r.company_id)
    ...
    def _process_avg_price_multicompany(self, templates):
        table = self.env["res.currency"]._get_simple_currency_table(self.env.companies)
        query = SQL(
            """
                SELECT
                    pp.product_tmpl_id,
                    ...
                    SUM(
                         ...
                         *
                        (CASE COALESCE(account_currency_table.rate, 0) WHEN 0 THEN 1.0
                        ELSE account_currency_table.rate END)
                    ) / NULLIF(COUNT(pol.id), 0) AS average_price
                FROM purchase_order_line pol
                         JOIN purchase_order po ON pol.order_id = po.id
                         JOIN product_product pp ON pol.product_id = pp.id
                         JOIN product_template pt ON pp.product_tmpl_id = pt.id
                         JOIN %s ON account_currency_table.company_id = po.company_id
