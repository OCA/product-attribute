...
class sale_order_line(osv.osv):
    ...
    def product_id_change(
            ...
            fiscal_position=False, lang=False, update_tax=True):
        res = super(sale_order_line, self).product_id_change(
            cr, uid, ids, pricelist, product,
            qty=qty, uom=uom, qty_uos=qty_uos, uos=uos, name=name,
            partner_id=partner_id, fiscal_position=fiscal_position, lang=lang,
    ...
    def size_change(self, cr, uid, ids, size_x, size_y, size_z, product_id,
                    qty, context=None):
        ...
        if size_y and size_z:
            res['product_uos_qty'] = (
                size_x * size_y * size_z * p.weight * qty / 1000000
            )
        else:
        ...
        for sale in self.browse(cr, uid, ids):
            for line in sale.order_line:
                if line.prodlot_id.id:
                    self.pool.get('stock.production.lot.reservation').create(
                        cr, uid, {
                            'name': sale.name,
                            'size_x': line.size_x,
                            'size_y': line.size_y,
