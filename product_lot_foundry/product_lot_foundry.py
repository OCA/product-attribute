    ...
    def _get_size(dtype):
        def calc_date(self, cr, uid, context={}):
            if context.get('product_id', False):
                product = pooler.get_pool(cr.dbname).get(
                    'product.product').browse(cr, uid, [context['product_id']])[0]
        ...
        for move in self.browse(cr, uid, ids):
            ...
            if move.state in ('confirmed', 'waiting'):
                if move.product_id.cutting:
                    ...
                    pickings[move.picking_id.id] = 1
                    cr.execute('update stock_move set location_id=%s where id=%s',
                    ...
                    res = self.pool.get('stock.location')._product_reserve(cr, uid, [
                        move.location_id.id], move.product_id.id, move.product_qty, {'uom': move.product_uom.id})
                    if res:
                        done.append(move.id)
                        pickings[move.picking_id.id] = 1
                        r = res.pop(0)
