...
class SaleOrderLine(models.Model):
    ...
    @api.multi
    @api.depends('pack_line_ids', 'pack_line_ids.price_subtotal')
    def _get_pack_total(self):
        for s in self:
            pack_total = 0.0
            if s.pack_line_ids:
                pack_total = sum(x.price_subtotal for x in s.pack_line_ids)
                s.pack_total = pack_total

    @api.multi
    @api.onchange('pack_total')
    def _onchange_pack_line_ids(self):
        for s in self:
            s.price_unit = s.pack_total

    @api.multi
    @api.constrains('product_id', 'pack_type')
    def expand_none_detailed_pack(self):
        for s in self.filtered(lambda r: (r.pack_type ==
                                          'none_detailed_assited_price')):
            # Remove previous existing lines
            s.pack_line_ids.unlink()

            # Create a sale pack line for each product pack line
            for pack_line in s.product_id.pack_line_ids:
