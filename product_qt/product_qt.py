...
class testing_result(osv.osv):
    ...
    _columns = {
        ...
        'type': fields.selection(
            [
                ('rw_mat', 'Raw Material Testing'),
                ('in_prod', 'During Production Testing'),
                ('finish_prod', 'Finish Goods Testing')
            ],
            'Testing Type',
            readonly=True,
            select=True
...
class quality_test_config(osv.osv):
    ...
    _columns = {
        ...
        'min_limit': fields.float(
            'Min Limit',
            help='Minimum Limit of measure'
        ),
        'max_limit': fields.float(
            'Max Limit',
            help='Maximum Limit of measure'
        ),
...
class product_product(osv.osv):
    ...
    _columns = {
        'raw_m_test': fields.one2many(
            'quality.test.config',
            'product_idr',
            'Raw material testing',
            help="Quality Testing configuration for raw material."
        ),
        'production_test': fields.one2many(
            'quality.test.config',
            'product_idp',
            'During Production testing',
            help="Quality Testing configuration during production."
        ),
        'finished_test': fields.one2many(
            'quality.test.config',
            'product_idf',
            'Finished Goods testing',
            help="Quality Testing configuration for finished goods."
        ),
    }
product_product()
