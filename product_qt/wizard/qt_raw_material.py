...
def _get_cases(self, cr, uid, data, context):
    move_obj = pooler.get_pool(cr.dbname).get(
        'stock.move').browse(cr, uid, data['id'])
    prod_obj = pooler.get_pool(cr.dbname).get(
        'product.product').browse(cr, uid, move_obj.product_id.id)
    fields.clear()

    arch_lst = ['<?xml version="1.0"?>', '<form string="Quality Testing">']

    arch_lst.append('<field name="test_date"/>')
    fields['test_date'] = {'string': 'Testing Date', 'type':
                           'date', 'default': get_default(time.strftime('%Y-%m-%d'))}

    arch_lst.append('<field name="tester" colspan="1"/>')
    fields['tester'] = {
        'string': 'Tester', 'type': 'many2one', 'relation': 'hr.employee'}
    ...
    if prod_obj.raw_m_test:
        ...
        for case in prod_obj.raw_m_test:
            ...
            fields['actual' + '%s' %
                   case.name.id] = {'string': 'Actual', 'type': 'float'}
            arch_lst.append(
...
def check(self, cr, uid, data, context):
    ...
    move = pooler.get_pool(cr.dbname).get('stock.move')
    move_obj = move.browse(cr, uid, data['id'])
    prod_obj = pooler.get_pool(cr.dbname).get(
        'product.product').browse(cr, uid, move_obj.product_id.id)
    ...
    if prod_obj.raw_m_test:
        flag = False
        res = {}
        res = {'type': 'rw_mat', 'product': data['form']['product'], 'tester': data[
            'form']['tester'], 'test_date': data['form']['test_date']}
        ...
        if flag:
            move.write(cr, uid, data['id'], {'qlty_test_reject': True})
            move.write(cr, uid, data['id'], {'qlty_test_accept': False})
            move.write(cr, uid, data['id'], {'state': 'confirmed'})
        else:
            move.write(cr, uid, data['id'], {'qlty_test_accept': True})
            move.write(cr, uid, data['id'], {'qlty_test_reject': False})

    return {}


class wizard_qty_test(wizard.interface):
