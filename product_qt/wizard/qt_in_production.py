...
def _get_cases(self, cr, uid, data, context):
    wrk_obj = pooler.get_pool(cr.dbname).get(
        'mrp.production.workcenter.line').browse(cr, uid, data['id'])
    prod_obj = pooler.get_pool(cr.dbname).get('product.product').browse(
        cr, uid, wrk_obj.production_id.product_id.id)
    fields.clear()
...
def check(self, cr, uid, data, context):
    ...
    if prod_obj.production_test:
        flag = False
        res = {}
        res = {'type': 'in_prod', 'product': data['form']['product'], 'tester': data[
