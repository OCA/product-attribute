...
def _get_cases(self, cr, uid, data, context):
    mrp_obj = pooler.get_pool(cr.dbname).get(
        'mrp.production').browse(cr, uid, data['id'])
    prod_obj = pooler.get_pool(cr.dbname).get(
        'product.product').browse(cr, uid, mrp_obj.product_id.id)
    fields.clear()
...
def check(self, cr, uid, data, context):
    ...
    if prod_obj.finished_test:
        flag = False
        res = {}
        res = {'type': 'finish_prod', 'product': data['form']['product'], 'tester': data[
