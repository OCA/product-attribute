...
class product_price_fields(Model):
    ...
    _columns = {
        ...
        'field_name': fields.related(
            ...
            string='Field Name',
            store=True,
            help="you can chose the field name by default it will be build "
                 "with the name of the field replacing the space by '_' and "
                 "adding x_pm_ add the start"
        ),
        ...
        'sequence': fields.integer(
            'Sequence',
            help="The product 'variants' code will use order the field on the "
                 "product view"
        ),
        'tax_included': fields.boolean('Tax included'),
        'basedon_field_id': fields.many2one(
            'ir.model.fields',
            'Based on Field ID',
            domain=[
        ...
        'product_coef_field_id': fields.many2one(
            'ir.model.fields',
            'Product Coef Field ID',
            domain=[('model', '=', 'product.product'), ('ttype', '=', 'float')]
        ),
        'categ_coef_field_id': fields.many2one(
            'ir.model.fields',
            'Category Coef Field ID',
            domain=[
                ('model', '=', 'product.category'),
                ('ttype', '=', 'float')
            ]
        ),
        'default_basedon': fields.selection(
            [
                ('categ_coef', 'Price on category coefficient'),
                ('product_coef', 'Price on product coefficient'),
                ('manual', 'Manual price')
            ], 'Based on by default', required=True
        ),
        ...
        'inc_price_field_id': fields.many2one(
            'ir.model.fields',
            'Price Included Field ID',
            domain=[('model', '=', 'product.product'), ('ttype', '=', 'float')]
        ),
    }
    ...
    def _create_price_type(self, cr, uid, vals, context=None):
        field = (
            vals.get('field_name', 'x_pm_price_') +
            vals['name'].replace(' ', '_').replace('-', '_')
        ).lower()
        name = (
            _('Price') + ' ' +
            vals.get('field_name', vals['name']).capitalize()
        )
        type_obj = self.pool.get('product.price.type')
        price_type_ids = type_obj.search(
            cr, uid, [
                ('name', '=', name),
                ('field', '=', field),
            ], context=context
        ...
        if not price_type_ids:
            ...
            price_type_id = type_obj.create(
                cr, uid, type_vals, context=context
            )
        else:
    ...
    def create(self, cr, uid, vals, context=None):
        product_model_id = self.pool.get('ir.model').search(
            cr, uid, [('model', '=', 'product.product')]
        )[0]
        categ_model_id = self.pool.get('ir.model').search(
            cr, uid, [('model', '=', 'product.category')]
        )[0]
        if vals.get('price_field_id', False):
            return super(product_price_fields, self).create(
                cr, uid, vals, context
            )
        exist_id = False
        if vals.get('field_name', False):
            print 'field name'
            exist_id = self.pool.get('ir.model.fields').search(
                cr, uid, [('name', '=', vals['field_name'])]
            )
            if not exist_id and vals['field_name'][0:2] != 'x_':
                raise except_osv(
                    _('User Error'),
                    _("Please prefix the field name by x_ as it is a custom "
                      "field")
                )
        field_list = [
            'price',
        ...
        for field in field_list:
            if exist_id and field == 'price':
                ...
                field_vals = {
                    'name': (
                        vals.get('field_name', 'x_pm_') + field + '_' +
                        vals['name'].replace(' ', '_').replace('-', '_')
                    ).lower(),
                    'model_id': model_id,
                    'model': model,
                ...
                if field == 'basedon':
                    ...
                    field_vals['selection'] = (
                        "[('categ_coef','Price on category coefficient'),"
                        "('product_coef','Price on product coefficient'),"
                        "('manual','Manual price')]"
                    )
                    field_vals['required'] = True
                vals[field + '_field_id'] = self.pool.get('ir.model.fields')\
                    .create(cr, uid, field_vals)
                default_vals = {}
                if field == 'basedon' and vals.get('default_basedon', False):
                    ...
                    self.pool.get('product.product').write(
                        cr, uid, product_ids, default_vals, context=context
                    )
        if not exist_id:
            if not vals.get('currency_id'):
                vals['currency_id'] = self._get_currency(cr, uid, context)
            price_type_id = self._create_price_type(
                cr, uid, vals, context=context
            )
            self._create_price_list(
                cr, uid, vals, price_type_id, context=context
            )
        if vals.get('field_name', False):
            del vals['field_name']
        del vals['name']
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        return super(product_price_fields, self).unlink(
