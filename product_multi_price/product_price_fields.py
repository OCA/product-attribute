...
class product_price_fields(Model):
    ...
    def _create_price_type(self, cr, uid, vals, context=None):
        field = (vals.get('field_name', False) or 'x_pm_price_' +
                 vals['name'].replace(' ', '_').replace('-', '_')).lower()
        name = (_('Price')) + ' ' + \
            (vals.get('field_name', False) or vals['name']).capitalize()
    ...
    def create(self, cr, uid, vals, context=None):
        product_model_id = self.pool.get('ir.model').search(
            cr, uid, [('model', '=', 'product.product')])[0]
        categ_model_id = self.pool.get('ir.model').search(
            cr, uid, [('model', '=', 'product.category')])[0]
        ...
        if vals.get('field_name', False):
            ...
            if not exist_id and vals['field_name'][0:2] != 'x_':
                raise except_osv(_('User Error'), _(
                    "Please prefix the field name by x_ as it is a custom field"))
        field_list = [
            'price', 'inc_price', 'basedon', 'product_coef', 'categ_coef']
        ...
        for field in field_list:
            if exist_id and field == 'price':
                ...
                if field == 'basedon':
                    ...
                    field_vals[
                        'selection'] = "[('categ_coef','Price on category coefficient'),('product_coef','Price on product coefficient'),('manual','Manual price')]"
                    field_vals['required'] = True
                vals[
                    field + '_field_id'] = self.pool.get('ir.model.fields').create(cr, uid, field_vals)
                default_vals = {}
        ...
        if not exist_id:
            ...
            price_list = self._create_price_list(
