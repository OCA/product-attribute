...
class product_product(Model):
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type='form',
        ...
        if view_type == 'form' and result.get('model') == 'product.product' \
                and '<field name="list_price" modifiers="{}"/>' in \
                result['arch']:
            ...
            # select separator just before field :
            # list_price in order to remove it
        ...
        for field in product_price_fields_obj.browse(
            ...
            if not defaults.get(field.basedon_field_id.name):
                if field.default_basedon:
                    defaults[field.basedon_field_id.name] = \
                        field.default_basedon
        return defaults
        ...
        if read and not context.get('simple_read'):
            ...
            for field in fields:
                ...
                if field[0:11] == 'x_pm_price_':
                    ...
                    elif 'x_pm_price_' + name not in fields:
                        fields.append('x_pm_price_' + name)
                        add_fields.append('x_pm_price_' + name)
                    if 'x_pm_inc_price_' + name not in fields:
                        fields.append('x_pm_inc_price_' + name)
                        add_fields.append('x_pm_inc_price_' + name)
            ...
            for result in results:
                ...
                for field in fields:
                    ...
                    if name:
                        ...
                        tax_inc = False
                        price_field_ids = prod_price_fields_obj.search(
                            cr, uid, [
                                ('field_name', '=', price_name)
                            ], context=context)
                        if price_field_ids:
                            price_field = prod_price_fields_obj.read(
                                cr, uid, price_field_ids[0], ['tax_included'],
                                context=context
                            )
                        ...
                        if result['x_pm_basedon_' + name] == 'manual':
                            if result['standard_price']:
                                if tax_inc and result.get('taxes_id'):
                                    if not tax.related_inc_tax_id:
                                        raise except_osv(
                                            _("No related tax"),
                                            _("You need to define a related "
                                              "included tax for the sale tax!")
                                        )
                                    taxes = tax_obj.compute_all_with_precision(
                                        cr, uid, [tax.related_inc_tax_id],
                                        result['x_pm_inc_price_' + name], 1,
                                        precision=6
                                    )
                                    result[price_name] = taxes['total']
                                elif result.get('taxes_id'):
                                    taxes = tax_obj.compute_all_with_precision(
                                        cr, uid, [tax], result[price_name], 1,
                                        precision=6
                                    )
                                    result['x_pm_inc_price_' + name] = \
                                        taxes['total_included']
                                else:
                                    result['x_pm_inc_price_' + name] = \
                                        result[price_name]
                                result['x_pm_product_coef_' + name] = \
                                    result[price_name] / \
                            ...
                            categ = self.pool.get('product.category').read(
                                cr, uid, [categ_id],
                                ['x_pm_categ_coef_' + name], context=context
                            )
                            if categ[0].get('x_pm_categ_coef_' + name):
                                result['x_pm_product_coef_' + name] = \
                                    categ[0]['x_pm_categ_coef_' + name]
                            else:
                        ...
                        if result['x_pm_basedon_' + name] in \
                                ['categ_coef', 'product_coef']:
                            if result.get('taxes_id'):
                                taxes = tax_obj.compute_all_with_precision(
