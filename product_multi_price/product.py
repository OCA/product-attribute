...
# print result
            ...
            # select separator just before field : list_price in order to
            # remove it
            ...
            if btn:
                ...
                _button = etree.SubElement(
                    ...
                    type="object")
                for field in product_price_fields_obj.browse(cr, uid, product_price_fields_ids, context=context):
                    price_fields = [field.field_name, field.basedon_field_id.name,
                                    field.product_coef_field_id.name, field.inc_price_field_id.name]
    ...
    def default_get(self, cr, uid, fields_list, context=None):
        defaults = super(product_product, self).default_get(
        ...
        for field in product_price_fields_obj.browse(cr, uid, product_price_fields_ids, context=context):
            if not defaults.get(field.basedon_field_id.name):
                if field.default_basedon:
                    defaults[
                        field.basedon_field_id.name] = field.default_basedon
        return defaults
    ...
    def read(self, cr, uid, ids, fields=None, context=None, load='_classic_read'):
        ...
        if read and not context.get('simple_read'):
            ...
            results = super(product_product, self).read(
            ...
            for result in results:
                ...
                for field in fields:
                    ...
                    if name:
                        ...
                        if price_field_ids:
                            price_field = prod_price_fields_obj.read(
                                cr, uid, price_field_ids[0], ['tax_included'], context=context)
                            if price_field.get('tax_included'):
                                tax_inc = True
                        if result.get('taxes_id'):
                            # TODO support several taxes ?
                            tax = tax_obj.browse(
                                cr, uid, result['taxes_id'][0], context=context)
                        if result['x_pm_basedon_' + name] == 'manual':
                            if result['standard_price']:
                                if tax_inc and result.get('taxes_id'):
                                    if not tax.related_inc_tax_id:
                                        raise except_osv(_("No related tax"), _(
                                            "You need to define a related included tax for the sale tax!"))
                                    taxes = tax_obj.compute_all_with_precision(
                                        cr, uid, [tax.related_inc_tax_id], result['x_pm_inc_price_' + name], 1, precision=6)
                                    result[price_name] = taxes['total']
                                elif result.get('taxes_id'):
                                    taxes = tax_obj.compute_all_with_precision(
                                        cr, uid, [tax], result[price_name], 1, precision=6)
                                    result[
                                        'x_pm_inc_price_' + name] = taxes['total_included']
                                else:
                                    result[
                                        'x_pm_inc_price_' + name] = result[price_name]
                                result[
                                    'x_pm_product_coef_' + name] = result[price_name] / result['standard_price']
                            else:
                                result['x_pm_product_coef_' + name] = 0
                        elif result['x_pm_basedon_' + name] == 'product_coef':
                            if result.get('x_pm_product_coef_' + name):
                                price_ex = result[
                                    'standard_price'] * result['x_pm_product_coef_' + name]
                            else:
                            ...
                            categ = self.pool.get('product.category').read(
                                cr, uid, [categ_id], ['x_pm_categ_coef_' + name], context=context)
                            if categ[0].get('x_pm_categ_coef_' + name):
                                result[
                                    'x_pm_product_coef_' + name] = categ[0]['x_pm_categ_coef_' + name]
                            else:
                        ...
                        if result['x_pm_basedon_' + name] in ['categ_coef', 'product_coef']:
                            if result.get('taxes_id'):
                                taxes = tax_obj.compute_all_with_precision(
                                    cr, uid, [tax], price_ex, 1, precision=6)
                                result[
                                    'x_pm_inc_price_' + name] = taxes['total_included']
                            else:
...
class product_category(Model):
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        result = super(product_category, self).fields_view_get(
            cr, uid, view_id, view_type, context, toolbar=toolbar, submenu=submenu)
        if view_type == 'form':
            product_price_fields_obj = self.pool.get('product.price.fields')
            product_price_fields_ids = product_price_fields_obj.search(
